...
client.on('message', message => {
	...
	// reacts to bot posting in ans channel
		...
		message.channel.name.endsWith('-ans') &&
		message.author.bot &&
	...
	// reacts to bot posting in private channel
		...
		for (i of subjects) {
			if (
				message.channel.name.startsWith(`${i}-questions`) &&
				message.channel.name !== `${i}-questions`
			) {
				message.react('❌');
				return;
			}
...
client.on('messageReactionAdd', async (reaction, user) => {
	...
	if (reaction.message.channel.name.endsWith('-ans')) {
		...
		for (i of subjects) {
			if (
				reaction.message.channel.name.startsWith(`${i}-questions`) &&
				reaction.message.channel.name !== `${i}-questions`
			) {
				client.commands
					.get('reactClose')
					.execute(reaction, user, `${i}-ans`, logs);
				return;
			}
		}
	}
